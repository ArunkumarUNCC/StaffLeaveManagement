<?php
	...
	if($type=="All"){
		...
		$get=mysql_query("SELECT a.`leave_type`,b.`staff_name`,a.`purpose`,b.`department`,b.`designation`,a.`dates` FROM $tb_name a,`leave_users` b WHERE a.`staff_id`='$id' AND a.`staff_id`=b.`staff_id` AND a.`principal`='Approve'");
		...
		$get1=mysql_query("SELECT a.`leave_type`,b.`staff_name`,a.`purpose`,b.`department`,b.`designation`,a.`dates` FROM $tb_name a,`leave_users` b WHERE a.`staff_id`='$id' AND a.`staff_id`=b.`staff_id` AND a.`principal`='Approve' AND a.`leave_type`='$type'");
