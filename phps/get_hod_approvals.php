<?php
	...
	$get=mysql_query("SELECT * FROM $tb_name where `staff_id` IN (SELECT `staff_id` FROM `leave_users` WHERE `department`='$hod_dept[0]') AND `staff` NOT LIKE '%-Pending%' AND `hod`='Pending'");
	
	while($row=mysql_fetch_array($get)){
		array_push($staff_ids,$row[0]);
		$name=mysql_query("SELECT `staff_name`,`photo` FROM `leave_users` WHERE `staff_id`='$row[0]'");
		while($row2=mysql_fetch_array($name))
			array_push($staff_names,$row2[0]);
			array_push($photos,$row2[0]);
	...
	echo json_encode(array($staff_ids,$staff_names,$dates,$sessions,$alternate_staff,$alternate_class,$alternate_period,$type,$total,$purpose,$alternate_subject,$alternate_date,$alternate_staff_names,$photos));
?>